...
class SttEngine {
  ...
  Future<void> init(Lang lang) async {
    ...
    try {
      ...
      _vad = sherpa.Vad(_vadConfig!);

      // Configure NeMo CTC recognizer (AI4Bharat IndicConformer).
      _recognizer = sherpa.OfflineRecognizer(
        config: sherpa.OfflineRecognizerConfig(
          modelConfig: sherpa.OfflineModelConfig(
            transducer: const sherpa.OfflineTransducerModelConfig(
              encoder: '',
              decoder: '',
              joiner: '',
            ),
            paraformer: const sherpa.OfflineParaformerModelConfig(
              encoder: '',
              decoder: '',
            ),
            zipformer2Ctc: const sherpa.OfflineZipformer2CtcModelConfig(
              model: '',
            ),
            nemoEncDecCtc: sherpa.OfflineNemoEncDecCtcModelConfig(
              model: modelPath,
            ),
            tokens: tokensPath,
            numThreads: 2,
          ...
          lmConfig: const sherpa.OfflineLMConfig(
            model: '',
            scale: 0.1,
          ),
    ...
    while (_vad!.isReady()) {
      ...
      if (segment.samples.isNotEmpty) {
        ...
        if (result.isNotEmpty) {
          onResult(result, true); // Final result
        }
        stream.free();
      }
